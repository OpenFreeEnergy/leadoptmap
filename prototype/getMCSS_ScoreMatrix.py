...
#inputmolfiles = glob.glob('../molecules_original/*.mol2') 
...
#(mcss, natoms) = determineMCSS_requireRings(molecules['1,1,1,2,2,2-hexachloroethane'], molecules['1,1,1,2,2-pentachloroethane'], debug = True)
#print "1-2:", natoms
#(mcss, natoms) = determineMCSS_requireRings(molecules['chloroethane'], molecules['chloroethane'], debug = True)
#print "2-2:", natoms
#raw_input()
...
for (idx, title1) in enumerate(titles):
    ...
    for title2 in titles[idx:]:
        ...
        if chg1 == chg2:
            ...
            (mcss, natoms) = determineMCSS_requireRings(mol1, mol2, debug = False)
            #Apply with looser options if no match found
            if natoms==0:
                (mcss, natoms) = determineMCSS_requireRings( mol1, mol2, debug = False, atomexpr = OEExprOpts_RingMember, bondexpr = OEExprOpts_ExactBonds | OEExprOpts_EqAromatic | OEExprOpts_EqSingleDouble )
        else: 
            (mcss, natoms) = None, 0
        ...
        if mcss: #Only if there is actually a match!
            ...
            joint_title=joint_title.replace(' ','_')
            writeMolecule( mcss, os.path.join( 'mcss', joint_title+'.mol2'), preserve_atomtypes = True)
...
file = open( 'titles_and_mcssscorearray_charges.pickle', 'w')
