...
class Mcs(object):
    ...
    def search_all(self, mols):
        """
                ...
                """
        raise NotImplementedError("`search_all' method not implemented in subclass")
...
try:
    ...
    class OeMcs(Mcs):
        ...
        def __init__(self, atom_expr = oechem.OEExprOpts_EqONS, bond_expr = oechem.OEExprOpts_BondOrder | oechem.OEExprOpts_EqSingleDouble | oechem.OEExprOpts_EqAromatic, ringchecking = 'Strict', min_num_atoms = 4, is_approximate = True, is_strict = True, is_basic = False):
        ...
        def search(self, mol0, mol1):
            ...
            if mol0.total_charge() == mol1.total_charge():
                mol0 = mol0._struc
                mol1 = mol1._struc
                for mols in (mol0,mol1):
                    for atom in mols.GetAtoms():
                        if atom.IsHydrogen():
                            mols.DeleteAtom(atom)
                if not self._is_approximate:
                    mcss = OEMCSSearch (mol1, self._atom_expr, self._bond_expr)
                else:
                    mcss = OEMCSSearch (mol1, self._atom_expr, self._bond_expr, OEMCSType_Approximate)
                ...
                for match in mcss.Match(mol0, True):
                    ...
                    mcs_mol = OEMol()
                    OESubsetMol(mcs_mol, match, True)
                    nmatched = 0
                    for mcs_atom in mcs_mol.GetAtoms():
                        if not mcs_atom.IsHydrogen():
                            nmatched+=1
                ...
                if mcs_mol:
                    ...
                    id0 = mol0.id()
                    id1 = mol1.id()
                    ret.append( self.deposit_to_kbase( id0, id1, atom_match0, atom_match1, mcs_mol ) )
            return ret
        ...
        def search_all(self, mols):
            ...
            for i in range(num_mol):
                for j in range(i):
                    single_search = self.search(mols[i], mols[j])
                    if single_search:
                        ret.append(single_search[0])
            return ret
...
try:
    ...
    class SchrodMcs(Mcs):
        ...
        def search_all(self, mols):
            ...
            if val != 0:
                msg = 'CanvasMCS exited prematurely. This could be because the input molecules were too dissimilar or too numerous, or because the chosen atom-typing scheme was too general.'
